...
# see: http://www.swissbib.org/wiki/index.php?title=SRU
...
while ( $row = $csv->getline($fh_in) ) {
    ...
    ######################################################################
    # START SEARCH ON SWISSBIB SRU Service
    # Documentation: http://www.swissbib.org/wiki/index.php?title=SRU    
    ######################################################################
    ...
    ##################################################
    # Handle bad results: case "no results" 
	##################################################
    ...
    ##################################################
    # Handle bad results: case "more than 10 records"
	##################################################
    ...
    if ( $numberofrecords >= 1 && $numberofrecords <= 10 ) {
        ...
        foreach $rec (@record) {
            ...
            ############################################################
            # Get Swissbib System Nr., Field 001 for export
